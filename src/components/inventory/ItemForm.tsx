...
import { Item, Category, EnhancedItem } from '../../lib/types';
import LoadingSpinner from '../ui/LoadingSpinner';
import { AlertCircle, DollarSign } from 'lucide-react';
import { useAuthStore } from '../../lib/store';

interface ItemFormProps {
  initialData?: Partial<EnhancedItem> | any;
  categories: Category[];
...
export default function ItemForm({ initialData, categories, onSubmit, onCancel }: ItemFormProps) {
  ...
  const [errors, setErrors] = useState<{ name?: string; description?: string; category_id?: string; unit_price?: string }>({});
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const unit_price = isAdmin ? parseFloat(formData.get('unit_price') as string) : (initialData as any)?.unit_price;
    ...
    <form onSubmit={handleSubmit} className="space-y-6 sm:space-y-8">
      ...
      >
        ...
        <select
          ...
          required
          className={`w-full input-dark input-large ${errors.category_id ? 'ring-error-500 border-error-500' : ''
            }`}
        >
      ...
      >
        ...
        <div className="relative">
          ...
          <input
            ...
            disabled={!isAdmin}
            className={`w-full input-dark input-large pl-10 ${errors.unit_price ? 'ring-error-500 border-error-500' : ''
        ...
        {!isAdmin && (
          <div className="mt-2 text-xs text-warning-400">
            Admin privileges required to edit price
          </div>
        )}
      </motion.div>
