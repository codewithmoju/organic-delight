...
function CustomerInitials({ name }: { name: string }) {
  const initials = name
    ...
    .toUpperCase();
  return (
    <div className="w-8 h-8 rounded-lg bg-error-500/15 flex items-center justify-center flex-shrink-0">
      <span className="text-xs font-bold text-error-600 dark:text-error-400">{initials}</span>
    </div>
  );
}
...
export default function CustomerCreditWidget({ customers, isLoading }: CustomerCreditWidgetProps) {
  ...
  return (
    ...
    >
      {/* Gradient accent */}
      <div className="absolute top-0 left-0 w-full h-0.5 bg-gradient-to-r from-error-500/50 via-error-500/20 to-transparent" />
      ...
      {/* Summary pill */}
      {count > 0 && (
        <div className="flex items-center gap-2 mb-3 px-3 py-2 rounded-xl bg-gradient-to-r from-error-500/10 to-error-500/5 border border-error-500/20">
      ...
      {withBalance.length === 0 ? (
        <div className="text-center py-4">
          <div className="w-10 h-10 rounded-full bg-success-500/10 flex items-center justify-center mx-auto mb-2">
            <AlertCircle className="w-5 h-5 text-success-500" />
          </div>
        ...
        <div className="space-y-2">
          {withBalance.map((customer, i) => (
            <motion.button
              ...
              className="w-full flex items-center gap-3 p-3 rounded-xl bg-error-500/5 border border-error-500/20 hover:bg-error-500/10 hover:border-error-500/30 transition-all text-left group"
            >
