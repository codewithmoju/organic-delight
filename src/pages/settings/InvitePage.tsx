...
import { UserPlus, Copy, Check, Trash2, Clock, ArrowLeft, Mail } from 'lucide-react';
import { toast } from 'sonner';
import { useNavigate } from 'react-router-dom';
import { useAuthStore } from '../../lib/store';
import { createInvite, getPendingInvites, cancelInvite } from '../../lib/api/invites';
  ...
  const handleSendInvite = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        ...
        toast.success(`Invite created for ${email.trim()}. Link copied — share it with them.`);
      } catch {
        toast.success(`Invite created for ${email.trim()}. Copy the link below to share.`);
      }
      setEmail('');
      load();
    } catch (err: any) {
    ...
    <div className="max-w-5xl mx-auto space-y-4 sm:space-y-6 pb-16">
      ...
      >
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            <h2 className="text-lg font-semibold text-foreground">Create Invite</h2>
            <p className="text-xs text-muted-foreground mt-0.5">A link will be generated — share it with the person you want to invite.</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div className="flex items-end">
            <motion.button
              ...
              whileTap={{ scale: 0.98 }}
              className="btn-primary w-full h-12 rounded-xl flex items-center justify-center gap-2 font-medium shadow-lg shadow-primary/20 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {isSending ? (
      ...
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
        ...
        {isLoading ? (
          ...
          <EmptyState
            ...
            description="All invites have been accepted or expired."
          />
        ) : (
